    ...
    def load_db():
        ...
        try:
            with open(DB_FILE, "r", encoding='utf-8') as f:
                data = json.load(f)
                if "settings" not in data: data["settings"] = {"usd_rate": 4.45}
                if "leave_records" not in data: data["leave_records"] = []
                for emp_id, emp_data in data['employees'].items():
                    if "status" not in emp_data: emp_data["status"] = "Active"
                    if emp_data["status"] == "Resigned": emp_data["status"] = "Inactive"
                    if "basic_salary" not in emp_data: emp_data["basic_salary"] = 0.0
                    if "last_increment" not in emp_data: emp_data["last_increment"] = None
                    if "last_bonus" not in emp_data: emp_data["last_bonus"] = None
                    if "master_remark" not in emp_data: emp_data["master_remark"] = ""
                    if "date_of_birth" not in emp_data: emp_data["date_of_birth"] = None
                return data
